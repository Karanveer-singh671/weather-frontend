...
import Form from './components/ImageLinkForm/ImageLinkForm';
import './App.css';
    ...
    if(!city) {
      ...
      .then(weather => {
        ...
        this.setState({weather, show: true})
          ...
          this.setState({errors: "There was an Error getting the Requested City"})
        }
        })
      .catch(err => this.setState({errors: "There was an Error getting the Requested City"}));
  }
/** react-router-dom would be better for routing than this */
  ...
  render() {
    const { isSignedIn, route, city, country, units, errors,weather, show } = this.state;
    return (
      <div className="App">
